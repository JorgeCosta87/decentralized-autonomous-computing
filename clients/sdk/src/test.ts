...
async function main() {
  ...
  console.log('📝 Loading Test Keypairs...\n');
  ...
  try {
    ...
    async function registerNodeIfNeeded(
      ...
      if (existingNodeInfo) {
        ...
        console.log(`✅ ${nodeLabel} already registered!`);
        ...
        console.log(`✅ ${nodeLabel} registered successfully!`);
    ...
    console.error('❌ Failed to register nodes:');
    console.error(error);
    process.exit(1);
  }
  ...
  try {
    ...
    try {
      ...
      console.log(`✅ ${claimedNodes.length} public node(s) have been claimed and are in AwaitingValidation state!\n`);
      ...
      for (let i = 0; i < claimedNodes.length; i++) {
        ...
        console.log(`✅ ${nodeLabel} activated successfully!`);
        console.log(`  Transaction Signature: ${signature}\n`);

        // Verify activation
        const updatedNodeInfo = await dacClient.getNodeInfo(nodeKeypair.address);
        if (updatedNodeInfo) {
          console.log(`✅ Verified ${nodeLabel} activation:`);
          console.log(`  Status: ${updatedNodeInfo.status}`);
          console.log(`  Node Type: ${updatedNodeInfo.nodeType}\n`);
        }
      }

      console.log('✅ All nodes activated successfully!\n');
    } catch (error: any) {
      if (error.message?.includes('Timeout')) {
        console.log('⚠️  Timeout waiting for nodes to be claimed.');
    ...
    console.error('❌ Failed to activate nodes:');
    console.error(error);
    process.exit(1);
  }
    ...
    if (existingAgent) {
      ...
      if (createdAgent) {
        ...
        if (createdAgent.status === AgentStatus.Pending) {
          console.log('\n⚠️  Agent is in Pending state - needs validation');
  ...
  try {
    ...
    console.log(`\n✅ Found Active agent at slot ${activeAgentSlotId}!`);
